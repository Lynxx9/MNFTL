...
void FtlImpl_MNftl::allocate_new_current_block(Event &event)
{
    ...
    current_blocks.insert(current_block.block);   // ★ 關鍵
...
enum status FtlImpl_MNftl::write(Event &event)
{
    ...
    if (old_ppn != -1)
        event.set_replace_address(Address(old_ppn, PAGE));

    PMT[lbn][pmd_index][map_slot] = new_ppn;
    PMD[lbn][pmd_index] = new_ppn;
...
void FtlImpl_MNftl::cleanup_block(Event &event, Block *block)
{
    ...
    for (uint i = 0; i < BLOCK_SIZE; i++) {
        ...
        // update mapping (brute-force)
        for (auto &lbn_pair : PMT) {
            auto &pmts = lbn_pair.second;
            for (uint idx = 0; idx < pmts.size(); idx++)
                for (uint s = 0; s < pmts[idx].size(); s++)
                    if (pmts[idx][s] == (long)old_ppn) {
                        pmts[idx][s] = new_ppn;
                        PMD[lbn_pair.first][idx] = new_ppn;
                        goto done;
                    }
        }
done:;
